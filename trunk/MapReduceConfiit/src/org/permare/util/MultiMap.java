 ...
 * @param <V> Values type
 */
public class MultiMap<K, V> implements Serializable,  Map<K, V> {
     ...
     * @param values_initial_capacity initial capacity for the collection of values per key. 
     */
    ...
    /**
     * Constructs an empty MultiMap instance with a given initial capacity
     * for keys and for the values collection per key.
     * This means that we are expecting initially <i>keys_initial_capacity</i> 
     * keys and, for each key, we are expecting about <i>values_initial_capacity</i>
     * values. 
     * @param values_initial_capacity initial capacity for the collection of values per key. 
    ...
    /**
     * adds a new value to those associated to the key. More than one value 
     * can be associated with one key. 
    ...
    /**
     * returns the <i>n-th</i> key (token) in the key set. 
     * Key position is determined by the insertion order. 
    ...
    /**
     * returns all values associated with a given key. Note that a key may
     * have several values associated with it.
     * 
     * @param key token whose values we are looking for
     * @return values associated with the key or null, if the key is not 
    ...
    /**
     * Returns an iterator given access to the collection of values 
    ...
    /**
     * returns the first value V associated with the key. Note that other
     * values can be associated with this key. 
    ...
    /**
     * associates the specified value with a given key, <b>replacing</b> all previous 
     * values  by this one. Old values associated with this key are returned as
     ...
     * does not correspond to declared &lt;K,V&gt;, preventing it from being stored in this map.
     */
    @Override
   ...
   //   public Object put(K key, V value) {
     ...
     * <b>Note</b> that, since key may be associated with multiple values, 
     * all values related to a key will be returned in a Collection, producing
     * a Collection containing itself other Collection of values (one Collection
     * by key). Besides, similar to other Map implementations, 
     * the returned collection is backed by the map, 
     * so changes to the map are reflected in the collection, and vice-versa. 
     * 
     * @return Collection &lt; Collection &lt; V &gt; &gt; containg values associated to 
     * each key. 
     */
    @Override
